    ...
    <body class="bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] p-6">
        <div class="w-full lg:max-w-4xl max-w-[800px] bg-white dark:bg-[#161615] rounded-lg p-6">
            <div class="flex items-center justify-between mb-4">
                <h1 class="text-2xl font-medium">Crear equipo</h1>
            ...
            <form action="{{ route('equipos.store') }}" method="POST" class="space-y-3">
                @csrf

                <div>
                    <label class="block text-sm mb-1">Nombre</label>
                    <input name="nombre" value="{{ old('nombre') }}" required
                           class="w-full px-3 py-2 border border-[#e3e3e0] rounded-sm text-sm">
                </div>
                ...
                <div>
                    ...
                    <select name="estado" class="w-full px-3 py-2 border border-[#e3e3e0] rounded-sm text-sm">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="px-4 py-2 bg-[#1b1b18] text-white rounded-sm">Guardar</button>
